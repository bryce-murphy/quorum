...
- `instructions: string[]` -> paths to `exact`, glob patterns to `globs`, **resolved relative to the CONTAINING config file's directory** (P1, cross-architect review; first-party — see resolution note below), NOT repo-root. **Remote-URL entries** (`http://`/`https://`, which first-party `instructions` explicitly allows) are deterministically external -> **skip** (not a repo path a PR can edit; neither floor nor block).
- **Absolute and `~`-home instruction/`{file:}` paths FAIL CLOSED (P1)** — same shape as the claude-md rule: OpenCode allows absolute/`~` config paths, and one that resolves back into the checkout is a PR-editable repo file the resolver would never floor, so a floored `opencode.json/.jsonc` containing an absolute/`~` instruction or `{file:}` path is a **hard error -> verify blocks**, not a silent skip. Skip only a path provably outside the repo (a collapsed `..` escape).
- `{file:...}` **only in instruction-bearing fields** (`agent.*.prompt`, deprecated `mode.*.prompt`, and `command.*.template` — a command template is the prompt sent to the LLM, opencode.ai/docs/commands, so a `{file:}` in it loads that file; cross-family red-team Repro 1) -> its path to `exact`/`globs`. **Not** arbitrary strings: a provider `apiKey: "{file:~/.secrets/key}"` must not floor. **Out of scope (-> QRM-4.0 residual):** the command-prompt **`@file`** include (opencode.ai/docs/commands#file-references) is a *distinct* mechanism — auto-included and **repo-ROOT-relative** ("commands run in your project's root directory"), spanning both `command.*.template` and `.opencode/commands/**/*.md` — a different resolution base than `{file:}`; deferred with its base sourced, not implemented from an inferred rule (cross-family red-team Repro 2).
- **Resolution note (pinned, first-party — https://opencode.ai/docs/config#files):** `{file:path}` substitution paths are *"Relative to the config file directory"* or absolute/`~`. The extractor therefore resolves BOTH `instructions` entries and instruction-field `{file:}` paths against the containing config's directory (`dirname(configPath)` join, POSIX-collapse `./`+`..`, skip on repo-escape, then `normalizePath`), identical in shape to the claude-md rule. This closes a P1 under-floor: a nested `packages/a/opencode.jsonc` with `{file:prompts/review.md}` loads `packages/a/prompts/review.md`; resolving repo-root would floor the wrong path and miss it. First-party does not distinguish an `instructions` base from `{file:}`, and for a root config the two coincide; config-dir-relative is the conservative, consistent choice. Re-prototyped against real `normalizePath` (10/10, incl. the nested case, `../shared`, glob passthrough, remote-URL skip, escape skip, absolute/home block).
...
- OpenCode `references` object (local-dir/git-repo *availability*), manual `@refs` inside `AGENTS.md`/`CLAUDE.md`-as-AGENTS (first-party: opencode does NOT auto-parse these — they are agent-driven lazy loads), and git-repo/external-local instruction references are **deferred** and carried as a QRM-4.0 residual (see manifest). These are agent-steering *availability* surfaces, not the deterministic content-loading that `instructions`/`{file:}` are. (Remote-URL `instructions` are handled in-scope by the skip above.)
...
**Single shared enforcement path (P3, cross-architect review).** Reference resolution becomes tree-reading (and async in forge mode); `cmdVerify` and `cmdTier` must call ONE shared internal that resolves policy+references and computes the floor, so a future edit can't update `verify` while leaving `tier` on the 2-arg pure path. `cmdTier` currently prints `computeTierFloor` directly and is synchronous — it must move onto the shared path.

**Policy must annotate its reference-bearing floors (P2, cross-architect review).** The `reference_extractor` enum fails closed on an *unknown* value, but a floored reference-bearing rule that simply *omits* the annotation silently resolves no references — the same under-floor class. QRM-3.4 acceptance therefore requires the committed policy to annotate `**/CLAUDE.md`, `**/CLAUDE.local.md`, `**/opencode.json`, `**/opencode.jsonc` with their extractor. Longer-term (banked follow-up): a mechanical policy lint that fails validation when a known reference-bearing glob is present without its expected extractor (principle 2 — mechanical over discipline).
...
**One shared matcher (P2, cross-architect review).** Define a single `isReferencedPath(path, referencedFloors): boolean` (exact case-folded lookup OR glob match) and use it in BOTH `computeTierFloor`'s reference contribution AND `computeUncoveredPaths`'s exemption override, so the exact/case-fold/glob semantics cannot drift between two hand-written matchers. Coverage becomes: if covered, skip; else if not referenced AND matches an exempt glob, skip; else uncovered.
...
GPT cross-architect review DONE — six findings, all accepted (P1 opencode config-dir-relative resolution as must-fix; P2 shared `isReferencedPath`, P2 policy-annotation requirement, P2 opencode `references`/AGENTS.md residual to QRM-4.0; P3 fail-closed diagnostics contract, P3 shared enforcement path). P1 fix re-prototyped against the real `normalizePath` (10/10): nested `{file:}`/`instructions` -> config-dir (`packages/a/opencode.jsonc` + `{file:prompts/review.md}` -> `packages/a/prompts/review.md`), `../shared` in-repo parent, glob passthrough, remote-URL skip, escape skip, absolute/home block; root configs unchanged. First-party source: https://opencode.ai/docs/config#files , /docs/rules , /docs/references (retrieved 2026-07-04).
